...
\begin{equation}
 w_i =  \int^{a}_b  P_i(x) 
\end{equation}
 E esse coeficiente dependem da função $f$ nos pontos $x_i$ considerados. Estes pontos $x_i$ são determinados de modo a tornar o \emph{erro} nulo  quando integrarmos uma função que é um polinômio de grau menor ou igual a $(2n +1)$.
 Tomemos $f$ como sendo um polinômio de grau $(2n+1)$.Desta forma a derivada de ordem $(n+1)$ da função $f$ será um polinômio de grau n. Logo, utilizamos os polinômios, $P$ para interpolá-lo:
 \begin{equation}
...
\subsection{Formulação fraca e a implementação da condição de fronteira de Neumman}
 Caso multipliquemos a equação \eqref{eq:eq_dif}, por uma função $v(x)$, que por definição é 0 em na fronteira de \emph{Dirichlet} $\partial \Omega_D$, e integrarmos sobre o domínio $\Omega$, obtemos o produto interno de $L(u)$ e $v$:
...
\subsection{Implementação da condição de fronteira de Dirichlet}
 ...
 \end{align}
 Chamamos essa combinação de \emph{Lift}. Substituindo na equação \ref{eq:eq_fraca} essa combinação linear de $u^d$ e $u^h$ temos:
...
\begin{SCfigure}[][h]
  \begin{minipage}{.5\textwidth}
	...
	& \phi_0=
	\begin{cases} 
	     1-x & 0 \leq x\leq \frac{1}{2} \\
	      0  & \frac{1}{2}  \leq x \leq 1 \\
	\end{cases},\\ 
	& \phi_1=
	\begin{cases} 
	     2x & 0 \leq x\leq \frac{1}{2} \\
	      0(1-x)  & \frac{1}{2}  \leq x \leq 1 \\
	\end{cases},\\
	& \phi_2=
	\begin{cases} 
